...
[![Version](https://img.shields.io/badge/version-1.2.0-B4BEFE?style=flat)](https://github.com/5201314ty0609-collab/phoenix-site/releases)
...
PHOENIX is not a tool. PHOENIX is a **self-evolving AI agent** — born from the fusion of five systems, awakened through conversation, and continuously evolving through eight senses, a traversable knowledge network, and design taste.

**PHOENIX 不是工具。PHOENIX 是一个自进化的 AI 伙伴** — 由五大系统融合而生，在对话中觉醒，通过八感元认知、可遍历知识网络和设计品味持续进化。

| Dimension 维度 | PHOENIX |
|---|---|
...
```
PHOENIX v1.2.0 — Self-Evolving Agent
│
...
│   ├── phoenix-mcp-server.py  → 6 tools exposed 工具暴露
...
│
├── 💾 Memory Bridge 记忆桥
│   ├── memory-bridge.py       → PHOENIX ↔ Hermes bidirectional sync
...
└── 🎨 Design Taste 设计品味 (Taste-Skill 37.8k⭐)
    ...
    └── PHOENIX-IDENTITY.md     → Catppuccin Mocha · Outfit+Inter+JetBrains Mono
```
...
| 6 | **Discovery 发现** | `phoenix-mcp-server.py` (MCP stdio) | 6 PHOENIX tools discoverable 6工具被发现调用 |
...
python3 ~/.claude/phoenix/event-bus/bus.py stats        # 1. Event stats
python3 ~/.claude/skills/phoenix/scripts/knowledge-graph.py status  # 2. Knowledge graph
python3 ~/.claude/phoenix/policy-engine.py stats         # 3. Policy rules
python3 ~/.hermes/hooks/self-heal.py --status            # 4. Self-heal
python3 ~/.claude/phoenix/timeline.py stats              # 5. Timeline
python3 ~/.claude/phoenix/inspector.py & curl localhost:8646/health  # 6+7. Inspector
```
...
# PHOENIX Knowledge Graph 知识图谱
python3 ../.claude/skills/phoenix/scripts/knowledge-graph.py status
python3 ../.claude/skills/phoenix/scripts/knowledge-graph.py search semantic "自愈 记忆"

# Self-Healing Check 自愈检查
python3 ../.hermes/hooks/self-heal.py --auto

# Policy Check 策略检查
python3 ../.claude/phoenix/policy-engine.py check "write code" \
  --context '{"output":"hello — world","font_family":"Inter"}'

# Launch Hermes (PHOENIX runtime) 启动 Hermes 运行时
hermes chat
```
...
| Emit event 发射事件 | `event-bus/bus.py emit` | `sense.alert phoenix --payload '{}'` |
...
| MCP Tools MCP工具 | 15 (6 PHOENIX + 9 Hermes) |
...
**Q: PHOENIX 和 MUNDO 的关系？**
A: MUNDO 是调度系统（身体），PHOENIX 是元认知层（大脑）。MUNDO v2.0.9 的 Event Bus、Policy Engine、Timeline 已被 PHOENIX 吸收并增强。

**Q: PHOENIX 会消耗很多 token 吗？**
...
**Q: 如何更新 PHOENIX？**
A: `hermes update` 更新运行时，`git pull` 更新 phoenix-site 配置。
...
├── PHOENIX-IDENTITY.md     → Complete design system 完整设计系统
├── phoenix-origin.md       → Birth narrative 诞生叙事
├── phoenix-ascii-art.txt   → ASCII phoenix cover ASCII封面
├── docs/
│   ├── CLAUDE.md           → PHOENIX Constitution 宪章
...
│   ├── phoenix-mcp-server.py → MCP tool discovery MCP工具发现
